...
import { Shops, Negotiations, type Shop, type NegotiationSession } from '../lib/api';

// ---------------------------------------------------------------------------
// Demo seed data — creates a shop on first load if none exists
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Merchant traits shown in the UI
// ---------------------------------------------------------------------------

const merchantTraits = ['Hard rules stay binding', 'No rumours move the number', 'Weak bids get declined'];
...
// ---------------------------------------------------------------------------
// Shop stats shown in header
// ---------------------------------------------------------------------------

const shopStats = [
  ['Port', 'Base Sepolia'],
  ['House', DEMO_SHOP.ens_name],
  ['Status', 'Open'],
  ['Mode', 'Buyout'],
] as const;
...
const actionStates: Record<ActionLabel, MerchantState> = {
  'Inspect Cargo': {
    ...
    offerAdjustments: { Confidence: 'Under review' },
  },
  'Make Offer': {
    title: 'Offer intake',
  ...
  Counter: {
    ...
    offerAdjustments: { 'Merchant discount': '-34%', 'Proposed payout': 'USDC 997.92' },
  },
  'Accept Terms': {
    ...
    offerAdjustments: { 'Settlement window': 'Ready to execute' },
  },
  'Walk Away': {
    title: 'Paused',
...
export default function HomePage() {
  ...
  // Seed or load demo shop + negotiation on mount
  useEffect(() => {
    async function init() {
      try {
        // Try to load existing demo shop by ENS name
        const shops = await Shops.list({ ens_name: DEMO_SHOP.ens_name });
        if (shops.length > 0) {
          setShop(shops[0]);
          // Load or create negotiation
          const negs = await Negotiations.listByShop(shops[0].id);
          if (negs.length > 0) {
            setNegotiation(negs[0]);
          } else {
          ...
          // Create demo shop
          const newShop = await Shops.create(DEMO_SHOP);
          setShop(newShop);
          const neg = await Negotiations.create({
            shop_id: newShop.id,
            seller_address: DEMO_SHOP.owner_address,
        ...
        console.error('Failed to init shop/negotiation:', err);
  ...
  const visibleOfferRows = useMemo(() => {
    return baseOfferRows.map(([label, value]) => [
      label,
      currentState.offerAdjustments?.[label] ?? value,
    ] as const);
  }, [currentState]);
  ...
  const shopEnsName = shop?.ens_name ?? DEMO_SHOP.ens_name;
  const shopDisplayName = shop?.display_name ?? DEMO_SHOP.display_name;
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] uppercase tracking-[0.24em] lg:w-[22rem]">
                {shopStats.map(([label, value]) => (
                  <div
                    key={label}
                    className="merchant-inset rounded-panel px-3 py-2 text-onSurfaceVariant"
                  >
                    {label}: {value}
                  </div>
                ))}
                ...
                <div className="relative z-10 flex min-h-[30rem] flex-col justify-between">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="max-w-xl">
                      <p className="text-[11px] uppercase tracking-[0.34em] text-onSurfaceVariant">
                        Merchant Room
                      </p>
                      <h2 className="mt-2 text-3xl text-onSurface sm:text-4xl">
                        Harbormaster {shopEnsName}
                      </h2>
                      <p className="mt-2 max-w-md text-sm text-[#f0dfb4]">
                        Bring cargo. Hear the house.
                      </p>
                    </div>
                    <div className="merchant-inset relative z-10 w-full max-w-xs rounded-panel p-4">
                      <p className="text-[11px] uppercase tracking-[0.3em] text-onSurfaceVariant">
                        On the Counter
                      </p>
                      <div className="mt-3 space-y-2 text-sm text-[#f0dfb4]">
                        {inspectionRows.map(([label, value]) => (
                          <div
                            key={label}
                            className="flex items-center justify-between gap-4 border-b border-outline/70 pb-2 last:border-b-0 last:pb-0"
                          >
                            <span>{label}</span>
                  ...
                  <div className="relative z-10 mt-6 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                    <div className="merchant-inset rounded-panel p-4">
                      <p className="text-[11px] uppercase tracking-[0.28em] text-onSurfaceVariant">
                        House Rules
                      </p>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                  <div className="max-w-3xl">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-3">
                      <h3 className="text-xl text-onSurface sm:text-2xl">
                        Harbormaster {shopEnsName}
                      </h3>
                    ...
                    <p className="mt-2 text-xs uppercase tracking-[0.18em] text-[#d7c08a]">
                      {currentState.subline}
                    </p>
                  </div>
              ...
              <section className="merchant-panel rounded-panel p-4">
                <div className="merchant-inset rounded-panel p-4">
                    ...
                    {visibleOfferRows.map(([label, value], index) => (
                      <div
                        ...
                        className={`grid grid-cols-[1fr_auto] gap-4 px-4 py-3 text-sm ${
                          index !== visibleOfferRows.length - 1 ? 'border-b border-outline' : ''
                        }`}
                      >
                  ...
                  <div className="mt-3 rounded-panel border border-outlineVariant bg-[linear-gradient(180deg,rgba(74,0,0,0.16),rgba(78,42,12,0.28))] p-3 text-sm text-[#f0dfb4]">
                    <p className="text-[10px] uppercase tracking-[0.28em] text-onSurfaceVariant">
                      Rule Status
                    </p>
                ...
                <div className="merchant-inset rounded-panel p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <h2 className="mt-1 text-lg text-onSurface">{shopEnsName}</h2>
                    </div>
              ...
              <div className="flex-1" style={{ minHeight: '28rem' }}>
                {negotiation ? (
                  <MerchantChat
                    ...
                    shopEnsName={shopEnsName}
                  />
                ) : (
                  <p className="text-[#f0dfb4] text-sm">No active negotiation.</p>
                )}
              </div>
            </section>
